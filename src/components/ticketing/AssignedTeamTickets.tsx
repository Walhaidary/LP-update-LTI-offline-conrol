...
export function AssignedTeamTickets() {
  ...
  const loadData = async () => {
    try {
      ...
      // Build query for tickets assigned to team members
      let query = supabase
        .from('ticket_details_view')
        .select('*')
        .eq('vendor_code', userProfile.vendor_code)
        .not('assigned_to', 'is', null)  // Only get tickets that are assigned
  ...
  return (
    <div className="p-6">
      <h2 className="text-lg font-medium mb-6">Tickets Assigned to my Team</h2>
