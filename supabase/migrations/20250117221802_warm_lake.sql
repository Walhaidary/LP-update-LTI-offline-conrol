...
BEGIN
  ...
  IF (TG_OP = 'INSERT' OR OLD.assigned_to IS DISTINCT FROM NEW.assigned_to) 
    ...
    INSERT INTO public.email_events (
      ...
      NEW.id, -- Use the ID directly from the NEW record
      NEW.ticket_number,
      assignee.email,
...
'Handles ticket assignment notifications by creating email events and sending emails. Uses ticket ID directly from the NEW record.';