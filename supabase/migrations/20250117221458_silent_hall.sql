...
BEGIN
  ...
  IF (TG_OP = 'INSERT' OR OLD.assigned_to IS DISTINCT FROM NEW.assigned_to) 
    ...
    -- Get the ticket ID from the tickets table
    DECLARE
      ticket_id uuid;
    BEGIN
      ...
      IF ticket_id IS NULL THEN
        RAISE EXCEPTION 'Could not find ticket ID for ticket number: %', NEW.ticket_number;
      END IF;
      ...
      INSERT INTO public.email_events (
        ...
        NEW.ticket_number,
        assignee.email,
        NEW.assigned_to::text, -- Store the UUID instead of the name
        email_subject,
        email_body,
        'created',
        auth.uid() -- No need to cast since column is now text
      );
...
'Handles ticket assignment notifications by creating email events and sending emails. Retrieves ticket ID from tickets table.';